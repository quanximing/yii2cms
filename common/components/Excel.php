...
class Excel
{
     ...
     * @param bool   print       设置打印格式
     * @param string freezePane  锁定行数，例如表头为第一行，则锁定表头输入A2
     * @param array  setARGB     设置背景色，例如['A1', 'C1']
     * @param array  setWidth    设置宽度，例如['A' => 30, 'C' => 20]
     * @param bool   setBorder   设置单元格边框
     * @param array  mergeCells  设置合并单元格，例如['A1:J1' => 'A1:J1']
     * @param array  formula     设置公式，例如['F2' => '=IF(D2>0,E42/D2,0)']
     * @param array  format      设置格式，整列设置，例如['A' => 'General']
     * @param array  alignCenter 设置居中样式，例如['A1', 'A2']
     * @param array  bold        设置加粗样式，例如['A1', 'A2']
     * @param string savePath    保存路径，设置后则文件保存到服务器，不通过浏览器下载
     */
    public static function exportExcel(array $datas,$fileName = '', array $options = [])
    {
        try {
            if (empty($datas)) {
                return false;
